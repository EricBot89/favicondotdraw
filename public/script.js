...
function makeCanvas() {
  ...
  cellArray.forEach((cellRow, cellRowIndex) => {
    cellRow.forEach((cell, cellIndex) => {
      ...
      cell.neighbors = cell.neighbors.filter(k => k != undefined);
    });
  });
}
...
function tooHandler(nodeToBeColored) {
  if (toolSelector.value === "paint") {
    colorize(nodeToBeColored);
...
function findColor() {
  ...
  if (!document.head.querySelector(`#${color.slice(1)}`)) {
    //check if we have generated this color class already
...
function recursiveFloodArea(originNode) {
  let oldColor = originNode.className;
  if (!(oldColor === findColor())) {
    colorize(originNode);
    originNode.neighbors.forEach(n => recursiveFloodArea(n));
  }
}
